...
export default function Tree_Drag2() {
  ...
  useEffect(() => {
    if (treeData) {
      treeData.map((item) => {
        if (item.id == dragedItem.id) setdragtree(item);
      });
    }
  }, [treeData]);
  ...
  return (
    <DndProvider backend={MultiBackend} options={getBackendOptions()}>
      <div>
                ...
                <div className=" tree-inner transition-all tree_textAndicon_parents hover:bg-slate-200 hover:shadow-md  rounded-lg p-2 flex items-center justify-between">
                  <span className="">
                    ...
                    <span className="text-gray-500">
                      {` (${numChild2})`}{" "}
                      <span className="hidden">{(numChild2 = 0)} </span>
                    </span>
                  </span>
        ...
        />
        {itemNode && deleteModal ? <Modal_Tree node={itemNode} /> : ""}
        {itemNode && addModal ? <AddExist node={itemNode} /> : ""}
        {itemNode && editModal ? <EditExist node={itemNode} /> : ""}
      </div>
    </DndProvider>
  );
}
