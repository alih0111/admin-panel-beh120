...
export default function AddExist({ addModal, node }) {
  ...
  const submitForm = async (e) => {
    ...
    if (checkVal) {
      ...
      let node2={'id':node.id,'text':node.text,"parent":id}
      ...
      id_url = node.id;
      let node2={'id':node.id,'text':node.text,"parent":node.parent,'droppable':true}
      // node2.NAME=node.NAME
      // node2.PARENTID=id
      try {
        // console.log(node2);
    ...
    fileObj = { id: id, text: newExist, parent: 0 };
